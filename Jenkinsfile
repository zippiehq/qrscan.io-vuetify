...
def projectName = 'qrscan.io-vuetify'
...
node {
  ...
  try {
    ...
    def pkgName = "${projectName}-${sanitizedBranchName}-${descCommit}-${env.BUILD_ID}"
    ...
    stage ("publish artifact") {
      ...
      sh "cd /tmp/${hostTempDir}/app/dist && aws s3 sync --delete . s3://z-dev-qrscan/public/"
    }
